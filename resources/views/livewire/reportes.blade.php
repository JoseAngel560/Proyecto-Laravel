...
<div class="container">
    ...
    <main class="inventario-container">
        <div class="tabs">
            ...
            <button wire:key="tab-devoluciones" class="tab-btn {{ $activeTab === 'devoluciones' ? 'active' : '' }}" wire:click="setActiveTab('devoluciones')">Devoluciones</button>
            ...
            <div class="filtros-container">
                ...
                @else
                    <div class="form-group">
                        ...
                        @error('facturaId') <span class="error">{{ $message }}</span> @enderror
                    </div>
                @endif
            </div>
                ...
                @else
                    <div class="form-group">
                        ...
                        <input type="number" id="compraId" wire:model.live="compraId" placeholder="Ingrese ID"
                            onkeydown="return !['e', 'E', '+', '-'].includes(event.key)"
                            oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                        @error('compraId') <span class="error">{{ $message }}</span> @enderror
                    </div>
                @endif
            </div>
        ...
        <div wire:key="content-devoluciones" id="devoluciones-tab" class="tab-content active">
            <div class="filtros-container">
                <div class="form-group">
                    <label>Filtrar por:</label>
                    <select wire:model.live="devolucionesFilterType" wire:key="devolucionesFilterType-{{ now()->timestamp }}">
                        <option value="date">Rango de Fechas</option>
                        <option value="id">ID Devolución</option>
                    </select>
                </div>
                @if($devolucionesFilterType === 'date')
                    <div class="form-group">
                        <label for="devolucionesStartDate">Fecha Inicio</label>
                        <input type="date" id="devolucionesStartDate" wire:model.live="devolucionesStartDate" wire:key="devolucionesStartDate-{{ now()->timestamp }}">
                    </div>
                    <div class="form-group">
                        <label for="devolucionesEndDate">Fecha Fin</label>
                        <input type="date" id="devolucionesEndDate" wire:model.live="devolucionesEndDate" wire:key="devolucionesEndDate-{{ now()->timestamp }}">
                    </div>
                @else
                    <div class="form-group">
                        <label for="devolucionId">ID Devolución</label>
                        <input type="number" id="devolucionId" wire:model.live="devolucionId" placeholder="Ingrese ID"
                            onkeydown="return !['e', 'E', '+', '-'].includes(event.key)"
                            oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                        @error('devolucionId') <span class="error">{{ $message }}</span> @enderror
                    </div>
                @endif
            </div>

            <div class="table-container" wire:loading.remove wire:target="setActiveTab, devolucionesFilterType, devolucionesStartDate, devolucionesEndDate, devolucionId">
                ...
                <table class="inventario-table">
                    ...
                    <tbody>
                        @if($devolucionesFilterType === 'date')
                            ...
                            <tr wire:key="devolucion-{{ $devolucion->id }}">
                                <td>{{ $devolucion->id }}</td>
                                <td>{{ $devolucion->fecha_devolucion ? \Carbon\Carbon::parse($devolucion->fecha_devolucion)->format('d/m/Y H:i') : 'N/A' }}</td>
                                <td>
                                ...
                                <td>C$ {{ number_format($devolucion->monto_total_devuelto, 2) }}</td>
                            </tr>
                            @endforeach
                        @else
                            @foreach($devoluciones as $devolucion)
                                @foreach($devolucion->detalles as $detalle)
                                <tr wire:key="devolucion-{{ $devolucion->id }}-detalle-{{ $detalle->id }}">
                                    <td>{{ $devolucion->id }}</td>
                                    <td>{{ $detalle->producto->id }}</td>
                                    <td>{{ $detalle->producto->nombre ?? 'N/A' }}</td>
                                    <td>{{ $detalle->producto->marca ?? '-' }}</td>
                                    ...
                                    <td>C$ {{ number_format($detalle->precio_unitario, 2) }}</td>
                                    <td>% {{ number_format($detalle->iva, 2) }}</td>
                                    <td>C$ {{ number_format($detalle->subtotal_devuelto, 2) }}</td>
                                </tr>
                                @endforeach
                            @endforeach
                        @endif
                ...
                @elseif(isset($devoluciones))
                <div class="alert alert-warning text-center">No hay devoluciones que coincidan con los filtros seleccionados.</div>
                @endif
            </div>
        </div>
        @endif
